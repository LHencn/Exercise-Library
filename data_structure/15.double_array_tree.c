...
void build(Node *root, DATNode *arr, int ind) {
    if (root == NULL) return ;
    arr[ind].base = getBase(root, arr);
    Node *p = root;
    if (p->flag) arr[ind].check = -arr[ind].check;
    for (int i = 0; i < BASE_CNT; i++) {
        if (p->next[i] == NULL) continue;
        arr[arr[ind].base + i].check = ind;
        build(p->next[i], arr, arr[ind].base + i);
    }
    return ;
}
...
int main() {
    ...
    printf("build end\n");
    for (int i = 0; i < MAX; i++) {
        printf("dat[%d].base = %d dat[%d].check = %d\n", i, dat[i].base, i, dat[i].check);
    }
